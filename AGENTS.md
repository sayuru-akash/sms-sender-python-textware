...
- `resources/input/`: raw CSV drops for one-off or batch cleaning
- `resources/output/`: cleaned CSV exports generated by the cleaner script
...
- Keep `resources/input/` and `resources/output/` folder placeholders tracked via `.gitkeep`, while leaving actual CSV contents ignored by git.
...
- Reuse sender helpers for Sri Lankan phone cleanup, name trimming, and email validation inside batch-cleaning paths.
...
If you modify the cleaner workflow, also verify at least one of:
